...
**When** a developer deletes a published `@AgenticField`, removes an operation, changes a parameter, removes an accepted input enum value, or adds a value to a closed response enum
...
- **FR-002**: The IR MUST record, per entity: the source class's qualified name (its identity),
  ...
  and element type, referenced entity if any, allowed values or enum constants, `openEnum`,
  ...
  enum constants when the type is an enum), return (Java type, return kind, referenced entity and
  DTO if any), and lifecycle. At document level: `irVersion`, `apiBasePath`, and `apiMajor` (the
...
- **FR-006**: `DtoGenerator`, `McpToolGenerator`, `RestControllerGenerator`, `OpenApiGenerator`
  ...
  `apiMajor`. Existing diagnostics MUST be preserved with the same kind and text: the "excluded
...
- **FR-009**: Output differences (entity fields and operation returns) MUST be classified as
  ...
  - Breaking:
    ...
    - its Java type or collection kind changed;
    - its `sensitive` changed from false to true;
    - an entity's `includeTypeInfo` changed;
    - a value was added to a field's enum constants or allowed values while the field's
      `openEnum` is false;
    - an operation's return Java type or return kind changed.
...
- **FR-012**: In the default (unlocked) mode, every breaking difference MUST be a compile ERROR.
  ...
  - the declaration that would legitimise it:
    - a removed or changed field → `@AgenticField(removedInVersion = <M+1>)` instead;
    - a removed or changed operation → `@AgenticExposed(apiUntil = <M>)` plus a replacement
      with `apiSince = <M+1>`;
...
- **FR-016**: The Gradle plugin's `agentic { }` extension MUST add two properties:
  ...
  `ai.atlas.contract.locked`. It MUST declare the baseline file as an optional input of Java
  compilation, so that creating, editing or accepting the baseline re-runs the gate.
...
- **FR-018**: The demo module MUST commit its baseline as `demo/.atlas/api.ir.json`, produced by
  ...
  `ai.atlas.contract.baseline` pointing at it. A demo test MUST assert that the committed
