...
| 2 | | Report a compile ERROR on each API-channel method of a service that maps to an already-mapped (HTTP method, path), naming path, method and the other `Class#method` (FR-002) | `modules/processor/src/main/java/com/egoge/ai/atlas/processor/AgenticProcessor.java` | Annotation Processor Engineer | [ ] |
...
| 5 | | Keep unique method names as `operationId`; qualify every shared one as `{ServiceSimpleName}_{methodName}_{httpMethod}` (FR-004) | `modules/processor/src/main/java/com/egoge/ai/atlas/processor/generator/OpenApiGenerator.java` | Annotation Processor Engineer | [ ] |
