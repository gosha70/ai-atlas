...
@ConditionalOnProperty(prefix = "ai.adam.mcp", name = "enabled", havingValue = "true", matchIfMissing = true)
        ...
        private ToolCallback[] resolveCallbacks() {
            ...
            for (Map.Entry<String, Object> entry : serviceBeans.entrySet()) {
                if (hasToolMethods(entry.getValue())) {
                    ...
                    log.info("AI-ADAM: Registered MCP tool bean: {}", entry.getKey());
                }
            }

            if (toolBeans.isEmpty()) {
                log.info("AI-ADAM: No @Tool-annotated beans found for MCP registration");
                return new ToolCallback[0];
            }

            log.info("AI-ADAM: Registered {} MCP tool bean(s)", toolBeans.size());
