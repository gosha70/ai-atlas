...
/**
 * Functional tests for the AI-ADAM Gradle plugin using Gradle TestKit.
 */
class AgenticPluginFunctionalTest {
    ...
    void pluginCreatesAgenticExtension() throws IOException {
        writeFile("build.gradle.kts", """
                ...
                tasks.register("printExtension") {
                    doLast {
                        val ext = project.extensions.getByType(ai.adam.plugin.AgenticExtension::class.java)
    ...
    void pluginAddsDependencies() throws IOException {
        ...
        assertThat(result.getOutput()).contains("impl: ai.adam:annotations:0.1.0");
        assertThat(result.getOutput()).contains("apt: ai.adam:processor:0.1.0");
        assertThat(result.getOutput()).contains("impl: ai.adam:runtime:0.1.0");
    }
